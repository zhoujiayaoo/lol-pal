...
def imageCompare(imagePath, logPath, threshold: int):
    ...
    :return:
    '''
    needCompareImgDict = {'a_1.jpg', 'a_2.jpg'}
    ...
    for imgs in needCompareImgDict.values():
        for i in range(len(imgs) - 1):
            ...
            img1 = cv2.resize(cv2.imread(img_1_path), (1920, 1080))  # 图片尺寸根据实际图片写入
            img2 = cv2.resize(cv2.imread(img_2_path), (1920, 1080))
...
if __name__ == '__main__':
    ...
    # imageCompare(imagePath, logPath, 1)
    img1 = cv2.resize(cv2.imread('screenshot/a_1.jpg'), (1920, 1080))  # 图片尺寸根据实际图片写入
    img2 = cv2.resize(cv2.imread('screenshot/a_1.jpg'), (1920, 1080))
    confidence = cal_ccoeff_confidence(img1, img2)
    print(confidence)
